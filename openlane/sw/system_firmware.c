...
static int jb_test(const char *label, const uint32_t key[4],
  ...
  if (!enc_ok) {
    ps("#     Debug ct: ");
    pc((ct[3] == exp_ct[3]) ? '1' : '0');
    pc((ct[2] == exp_ct[2]) ? '1' : '0');
    pc((ct[1] == exp_ct[1]) ? '1' : '0');
    pc((ct[0] == exp_ct[0]) ? '1' : '0');
    pc('\n');
    ps("#     Debug tg: ");
    pc((tag[1] == exp_tag[1]) ? '1' : '0');
    pc((tag[0] == exp_tag[0]) ? '1' : '0');
    pc('\n');
  }
  ln();
...
void test_xoodyak(int *pass) {
  ...
  XD(0x48) = 0;
  XD(0x4C) = 0;
  /* XD_CTRL: [17:16]=sel_type [12:8]=ad_len [4:0]=data_len */
  ...
  if (!(ct_ok && tag_ok)) {
    ...
    pc((ct[2] == exp_ct[2]) ? '1' : '0');
    pc((ct[1] == exp_ct[1]) ? '1' : '0');
    pc(((ct[0] & 0xFFFF0000) == (exp_ct[0] & 0xFFFF0000)) ? '1' : '0');
    pc('\n');
    ps("#     Debug tg: ");
    pc((tag[3] == exp_tag[3]) ? '1' : '0');
